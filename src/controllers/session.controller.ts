...
export const createSession = async (req: AuthenticatedRequest, res: Response): Promise<void> => {
    try {
        ...
        sendError(res, 'Failed to create session', [
...
export const getMySessions = async (req: AuthenticatedRequest, res: Response): Promise<void> => {
    try {
        ...
        const items = sessions.map((session) => ({
            ...
            myRole: session.hostId === userId ? ParticipantRole.HOST : ParticipantRole.PLAYER,
            host: {
                id: session.host.id,
                name: session.host.name,
            },
...
// =====================================
export const getSessionDetails = async (req: AuthenticatedRequest, res: Response): Promise<void> => {
    try {
        const userId = req.userId;
        ...
        const perPersonAmount = playerCount > 0 ? Math.round(totalAmount / playerCount) : 0;
        ...
        sendError(res, 'Failed to retrieve session', [
            { field: 'server', message: 'An error occurred' },
        ], 500);
    }
};
    ...
    try {
        ...
        if (session.hostId !== userId) {
            sendError(res, 'Permission denied', [
                { field: 'session', message: 'Only the host can modify this session' },
            ], 403);
            return;
        }
        ...
        sendError(res, 'Failed to update session', [
            { field: 'server', message: 'An error occurred' },
        ], 500);
    }
};
...
// =====================================
export const addPlayers = async (req: AuthenticatedRequest, res: Response): Promise<void> => {
    try {
        const userId = req.userId;
        const { sessionId } = req.params;
        ...
        if (session.status === SessionStatus.SPLIT_CONFIRMED || session.status === SessionStatus.CLOSED) {
            sendError(res, 'Cannot modify session', [
                { field: 'status', message: 'Cannot add players after split is confirmed' },
            ], 409);
            return;
        }

        const addedPlayers: Array<{ id: string; name: string; email: string | null; role: ParticipantRole }> = [];
...
// =====================================
export const removePlayer = async (req: AuthenticatedRequest, res: Response): Promise<void> => {
    try {
        const userId = req.userId;
...
// =====================================
export const addExpenses = async (req: AuthenticatedRequest, res: Response): Promise<void> => {
    try {
        const userId = req.userId;
        const { sessionId } = req.params;
...
// =====================================
export const generateSplit = async (req: AuthenticatedRequest, res: Response): Promise<void> => {
    try {
        const userId = req.userId;
        const { sessionId } = req.params;
...
// =====================================
export const getObligations = async (req: AuthenticatedRequest, res: Response): Promise<void> => {
    try {
        const userId = req.userId;
        const { sessionId } = req.params;
        ...
        const summary = {
            ...
            pending: obligations.filter((o) => o.status === 'PENDING' || o.status === 'MARKED_PAID').length,
            verified: obligations.filter((o) => o.status === 'VERIFIED').length,
        };

        sendSuccess(res, 'Obligations retrieved successfully', { obligations, summary });
    } catch (error) {
...
// =====================================
export const closeSession = async (req: AuthenticatedRequest, res: Response): Promise<void> => {
    try {
        const userId = req.userId;
        const { sessionId } = req.params;
        ...
        const pendingObligations = session.obligations.filter((o) => o.status !== 'VERIFIED');
        if (pendingObligations.length > 0) {
            sendError(res, 'Cannot close session', [
                { field: 'obligations', message: 'Not all payments have been verified' },
            ], 400);
            return;
        }
        ...
        const totalCollected = session.obligations.reduce((sum, o) => sum + Number(o.amount), 0);
...
// =====================================
export const uploadGroupPhoto = async (req: AuthenticatedRequest, res: Response): Promise<void> => {
    try {
        const userId = req.userId;
        const { sessionId } = req.params;
...
// =====================================
export const getFaceDetectionResults = async (req: AuthenticatedRequest, res: Response): Promise<void> => {
    try {
        const { sessionId } = req.params;
...
// =====================================
export const confirmFaceMappings = async (req: AuthenticatedRequest, res: Response): Promise<void> => {
    try {
        const userId = req.userId;
        const { sessionId } = req.params;
